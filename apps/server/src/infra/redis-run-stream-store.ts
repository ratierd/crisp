...
export class RedisRunStreamStore implements RunStreamStore {
  ...
  async markFinished(runId: string): Promise<void> {
    await this.client.xAdd(runKey(runId), '*', { e: END_SENTINEL });
  }
  ...
  async setActiveRun(conversationId: string, runId: string | null): Promise<void> {
    if (runId === null) await this.client.del(activeKey(conversationId));
    else await this.client.set(activeKey(conversationId), runId, { EX: RUN_TTL_SECONDS });
  }
