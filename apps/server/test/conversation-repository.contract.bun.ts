...
const implementations = [
  ...
  },
  {
    ...
    make: (): ConversationRepository & { close?: () => void } => new SqliteConversationRepository(':memory:'),
  },
];
...
describe(`ConversationRepository contract: ${name}`, () => {
  ...
  it('deleteMessagesAfter keeps the anchor, drops the rest (regenerate)', async () => {
    ...
    expect((await repo.get('c1', 'alice'))!.messages.map((m) => m.id)).toEqual(['m1', 'm2', 'm5']);
  });
  ...
  it('setFeedback finds the message by runId within the owner’s conversations', async () => {
    ...
    expect(await repo.setFeedback('run-a', { score: 'down', comment: 'meh' }, 'alice')).toBe(true);
    expect((await repo.get('c1', 'alice'))!.messages[0]!.feedback).toEqual({ score: 'down', comment: 'meh' });
  ...
  it('setFeedback refuses unknown runs and foreign owners with false, leaving votes intact', async () => {
    const repo = fresh();
    await repo.create(conversation('c1'), 'alice');
    await repo.appendMessage('c1', message('m1', { role: 'assistant', runId: 'run-a', feedback: { score: 'up' } }));
