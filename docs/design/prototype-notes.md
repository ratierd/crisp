...
- **Latency badge (meta line)** — appears under a *finished* assistant message, inside the prose
...
- **Streaming indicator (caret):** once tokens flow, the waiting row disappears and an inline
  ...
  the caret is intentionally *the only animation on the page* while running (plus the small pulse
...
```css
@keyframes crisp-caret{0%,55%{opacity:1}56%,100%{opacity:0}}
@keyframes crisp-pulse{0%,100%{opacity:.25}50%{opacity:1}}
```
...
.code-block-copy{
  ...
  font-family:var(--font-meta);font-size:10px;color:var(--text-3);cursor:pointer;
}
...
.code-block pre > div{min-height:1.65em} /* per-line rows; keeps blank lines tall */
```
...
```css
.prose table{border-collapse:collapse;margin:1.2em 0;width:100%}
.prose th{
  text-align:left;padding:7px 12px;border-bottom:1px solid var(--border);
  font-family:var(--font-ui);font-weight:600;font-size:12px;color:var(--text-2);
  letter-spacing:0.03em;
}
.prose td{
  padding:7px 12px;border-bottom:1px solid var(--border-faint);
  font-family:var(--font-meta);font-size:12px;color:var(--text);
}
```
