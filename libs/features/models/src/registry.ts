import type { KeyedProvider, Model } from './contracts';
import { keyedProviderOf } from './contracts';
...
export class ModelRegistry {
  constructor(private readonly keys: KeyConfig) {}
  ...
  async listModels(): Promise<Model[]> {
    ...
    return [...(this.keys.demoEnabled ? [DEMO_MODEL] : []), ...this.remoteModels()];
  }
  ...
  private remoteModels(): Model[] {
    return CATALOGS.flatMap((catalog) => {
      const hasServerKey = this.keys.serverKeyFor(catalog.id) !== null;
