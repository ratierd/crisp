...
import type { GatewayMessage, RunErrorKind, RunEvent } from './contracts';
import type { MessageStore, ModelGateway, RunStreamStore } from './ports';
...
export class RunService {
  ...
  async *execute(input: ExecuteRunInput): AsyncIterable<RunEvent> {
    const { gateway, messages, runStreams } = this.deps;
    const { runId } = input;
    const startedAt = this.now();

    if (input.userMessage) {
      await messages.appendMessage(input.conversationId, input.userMessage);
    }
    ...
    // to the message store. First failure wins; later writes are skipped.
    ...
    const finalize = async (stoppedEarly: boolean) => {
      if (text.length > 0 && !failed) {
        ...
        await messages.appendMessage(input.conversationId, assistant);
      }
